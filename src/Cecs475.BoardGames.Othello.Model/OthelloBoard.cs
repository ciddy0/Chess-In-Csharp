...
namespace Cecs475.BoardGames.Othello.Model {
	...
	public class OthelloBoard : IGameBoard {
		...
		/// </summary>
		private readonly struct FlipSet {
			/// <summary>
			/// The direction that flips were made.
			/// </summary>
			public BoardDirection Direction { get; init; }
			/// <summary>
			/// How many enemy pieces were flipped.
			/// </summary>
			public sbyte Count { get; init; }
		...
		private readonly List<OthelloMove> mMoveHistory = [];
		private readonly List<List<FlipSet>> mFlipSets = [];

		private int mAdvantageValue;

		private static readonly sbyte[,] mPositionWeights = new sbyte[BOARD_SIZE, BOARD_SIZE] {
			{16, 0, 8, 8, 8, 8, 0, 16},
			{0,  0, 1, 1, 1, 1, 0, 0},
			{8,  1, 2, 2, 2, 2, 1, 8},
			{8,  1, 2, 2, 2, 2, 1, 8},
		...
		void IGameBoard.ApplyMove(IGameMove move) {
			if (move is not OthelloMove m) {
				throw new ArgumentException("Can only apply OthelloMove objects");
			}
			ApplyMove(m);
		}
		#endregion
